...
<AccordionGroup>
  <Accordion title="Number Provisioning" icon="phone-plus">
    Purchase phone numbers directly through eigi.ai:
    - Local numbers in 190+ countries
    - Toll-free numbers
  ...
  <Accordion title="Number Assignment" icon="link">
    ...
    - One number per agent for simple routing
    - Multiple numbers to one agent
    - IVR routing (coming soon)
  </Accordion>
  
  <Accordion title="Caller ID" icon="id-card">
    Configure outbound caller ID display:
    - Use assigned number
    - Custom caller ID (where permitted)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Calls not connecting">
    ...
    - Check agent has a phone number assigned
  ...
  <Accordion title="Poor call quality">
    - Check your agent's voice provider settings
    - Ensure stable network connection
    - Review STT/TTS provider status
  </Accordion>
</AccordionGroup>
