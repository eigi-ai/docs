...
<Steps>
  ...
  <Step title="Test & Deploy">
    Test in sandbox mode, then go live
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Text-to-Speech (TTS)" icon="volume-high">
    Select your agent's voice:
    - **ElevenLabs** - Premium quality with voice cloning
    - **Cartesia** - Low-latency for real-time conversations
    - **Sarvam** - Indian language specialist
    - **OpenAI** - Reliable, integrated solution
  </Accordion>
  
  <Accordion title="Speech-to-Text (STT)" icon="ear-listen">
    Choose how your agent listens:
    - **Deepgram** - Industry-leading accuracy
...
<CardGroup cols={2}>
  ...
  <Card title="Outbound Agents" icon="phone-arrow-up-right">
    Make calls to customers proactively. Ideal for:
    - Sales outreach
...
<Tip>
**Start Simple**: Begin with a focused use case and expand capabilities over time.
</Tip>

<Tip>
**Test Thoroughly**: Run multiple test conversations covering different scenarios before going live.
</Tip>

<Tip>
**Iterate Based on Data**: Use call analytics to identify improvement opportunities.
</Tip>

<Warning>
Always test your agent with real-world scenarios before deploying to production.
</Warning>
