...
<AccordionGroup>
  <Accordion title="High-Value Leads Only" icon="star">
    ```
    IF lead_score > 80
    AND product_interest = "Enterprise"
    THEN send_slack_alert to #sales-leads
    ```
  </Accordion>
  ...
  <Accordion title="Urgent Issues Only" icon="exclamation">
    ...
    THEN send_email to support_manager@company.com
    ```
  </Accordion>
  
  <Accordion title="Business Hours Only" icon="clock">
    ```
    IF current_time BETWEEN 9:00 AND 18:00
    THEN send_slack_alert
    ELSE queue_for_digest
    ```
  </Accordion>
</AccordionGroup>
...
<Tip>
**Use Appropriate Channels**: Urgent issues → SMS/Slack. Detailed reports → Email. System integration → Webhooks.
</Tip>
...
<AccordionGroup>
  <Accordion title="Not receiving notifications">
    ...
    - Confirm notification triggers are configured
  ...
  <Accordion title="Webhook failures">
    ...
    - Ensure endpoint returns 2xx response quickly
    - Verify authentication headers are correct
  </Accordion>
  
  <Accordion title="Too many notifications">
    - Add condition filters to reduce volume
    - Switch to digest mode for non-urgent events
    - Adjust trigger thresholds (e.g., higher lead score)
  </Accordion>
</AccordionGroup>
