...
1. **Analysis prompt reference** (`call_analysis_prompt`) — Reference an existing analysis prompt by name and optionally version.
2. **Inline analysis prompt** (`call_analysis_prompt.custom_prompt`) — Provide prompt content directly to create a new analysis prompt version before linking it to the agent.
3. **Structured output schema** (`call_analysis_schema`) — Define the expected JSON structure for call analysis output.
